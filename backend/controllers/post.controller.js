...
exports.modifyPost = (req, res) => {
  ...
  Post.findOne({
    ...
    .then((post) => {
      // Si le post a une image, supprimer l'image du dossier '/images' et mettre à jour le post
      // Sinon mettre à jour le post directement
      if (req.file) {
        const filename = post.imageUrl.split("/images/")[1];
        updatedPost["imageUrl"] = `${req.protocol}://${req.get(
          "host"
        ...
        fs.unlink(`images/${filename}`, () => {
          Post.update(updatedPost, {
            ...
            .catch((error) =>
              res
                .status(400)
                .json({ message: "Impossible de supprimer cette photo", error })
            );
        });
      } else {
        Post.update(updatedPost, {
